...
// 1. Update copySeedImages handled by fix.js

// 2. Update image logic
const oldImageUrl = '\\`\\$\\{publicImageBase\\}/\\$\\{service.publicFile\\}\\`';
const newImageUrl = '(service.publicFile && service.publicFile.startsWith("http") ? service.publicFile : ' + oldImageUrl + ')';

seedCode = seedCode.split(oldImageUrl + ',').join(newImageUrl + ',');
seedCode = seedCode.split('const imageUrl = ' + oldImageUrl + ';').join('const imageUrl = ' + newImageUrl + ';');
...
let catMatch = seedCode.match(/const categories = \\[([\\s\\S]*?)\\];/);
if (catMatch) {
  ...
  seedCode = seedCode.replace(catMatch[0], 'const categories = [' + existingCats + '\\n];');
}

// 4. Inject subcategories
let subcatMatch = seedCode.match(/const subcategories = \\[([\\s\\S]*?)\\];/);
if (subcatMatch) {
  let existingSubcats = subcatMatch[1];
  ...
  seedCode = seedCode.replace(subcatMatch[0], 'const subcategories = [' + existingSubcats + '\\n];');
}

// 5. Inject services
let svcMatch = seedCode.match(/const services = \\[([\\s\\S]*?)\\];/);
if (svcMatch) {
  let existingSvcs = svcMatch[1];
  if (!existingSvcs.includes('Single Phase Breaker Replacement')) {
    ...
    seedCode = seedCode.replace(svcMatch[0], 'const services = [' + existingSvcs + '\\n];');
  }
}
